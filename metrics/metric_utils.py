...
def iterate_random_labels(opts, batch_size):
    if opts.G.c_dim == 0:
        ...
        while True:
            c = [dataset.get_label(np.random.randint(len(dataset)))
...
class ProgressMonitor:
    ...
    def update(self, cur_items):
        ...
        time_per_item = (cur_time - self.batch_time) / \
            max(cur_items - self.batch_items, 1)
        ...
        if (self.progress_fn is not None) and (self.num_items is not None):
            self.progress_fn(self.pfn_lo + (self.pfn_hi - self.pfn_lo)
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, swav=False, sfid=False,  **stats_kwargs):
    ...
    if opts.cache:
        ...
        cache_file = os.path.join(
            '.', 'dnnlib', 'gan-metrics', cache_tag + '.pkl')
        ...
        if opts.num_gpus > 1:
            flag = torch.as_tensor(
                flag, dtype=torch.float32, device=opts.device)
    ...
    # Main loop.
    item_subset = [(i * opts.num_gpus + opts.rank) %
                   num_items for i in range((num_items - 1) // opts.num_gpus + 1)]
    i = 0
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, swav=False, sfid=False, **stats_kwargs):
    ...
    progress = opts.progress.sub(
        tag='generator features', num_items=stats.max_items, rel_lo=rel_lo, rel_hi=rel_hi)
