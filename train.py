...
def launch_training(c, desc, outdir, dry_run):
    ...
    matching_dirs = [re.fullmatch(r'\d{5}' + f'-{desc}', x)
                     for x in prev_run_dirs if re.fullmatch(r'\d{5}' + f'-{desc}', x) is not None]
    # expect unique desc, continue in this directory
    if c.restart_every > 0 and len(matching_dirs) > 0:
        assert len(
            matching_dirs) == 1, f'Multiple directories found for resuming: {matching_dirs}'
...
# OC options
@click.option('--oc_type', help='Type of object-centric model to use ["SA", "DINOSAUR", "TI_SLOTS"]', type=str, default="DINOSAUR", show_default=True)
@click.option('--sa_size', help=' only if oc_type == "SA": if "large": use large encoder and decoder',  type=str, default="", show_default=True)
@click.option('--slot_dim', help='dimenstion of slots', type=click.IntRange(min=1), default=256, show_default=True)
@click.option('--oc_path', help='path to pretrained object-centric model', type=str, default="", show_default=True)
def main(**kwargs):
    # Initialize config.
    opts = dnnlib.EasyDict(kwargs)  # Command line arguments.
    c = dnnlib.EasyDict()  # Main config dict.
    c.G_kwargs = dnnlib.EasyDict(
        class_name=None, z_dim=64, w_dim=128, mapping_kwargs=dnnlib.EasyDict())
    ...
    # object-centirc
    oc_kwargs = {}
    oc_kwargs["oc_type"] = opts.oc_type
    oc_kwargs["slot_dim"] = opts.slot_dim
    oc_kwargs["sa_size"] = opts.sa_size
    oc_kwargs["oc_path"] = opts.oc_path
    c.oc_kwargs = oc_kwargs
    ...
    if opts.cfg == 'stylegan2':
        ...
        c.G_kwargs = dnnlib.EasyDict(class_name='pg_modules.networks_fastgan.Generator',
                                     cond=opts.cond, synthesis_kwargs=dnnlib.EasyDict())
    ...
    if os.path.isfile(last_snapshot):
        ...
        with dnnlib.util.open_url(last_snapshot) as f:
            cur_nimg = legacy.load_network_pkl(
                f)['progress']['cur_nimg'].item()
