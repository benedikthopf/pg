...
from torch_utils.manual_seed_context import ManualSeed
...
def training_loop(
    ...
    oc_kwargs={
        "slot_dim": 256,  # dimenstion of slots
        # type of object-centric model (SA, DINOSAUR, TI_SLOTS)
        "oc_type": "DINOSAUR",
        "sa_size": "",  # only if oc_type == "SA": if "large": use large encoder and decoder
        "oc_path": "",  # path to oc module
    }
):
    ...
    # will be broadcasted to exit loop
    __RESTART__ = torch.tensor(0., device=device)
    __CUR_NIMG__ = torch.tensor(
        resume_kimg * 1000, dtype=torch.long, device=device)
    ...
    slot_dim = oc_kwargs["slot_dim"]
    oc_type = oc_kwargs["oc_type"]
    sa_size = oc_kwargs["sa_size"]
    oc_path = oc_kwargs["oc_path"]
    common_kwargs = dict(c_dim=training_set.label_dim, slot_dim=slot_dim,
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        if ckpt_pkl is not None:            # Load ticks
            __CUR_NIMG__ = resume_data['progress']['cur_nimg'].to(
                device)
            __CUR_TICK__ = resume_data['progress']['cur_tick'].to(
                device)
            __BATCH_IDX__ = resume_data['progress']['batch_idx'].to(
                device)
            __PL_MEAN__ = resume_data['progress'].get(
    ...
    if rank == 0:
        ...
        slots = torch.empty(
            [batch_gpu, 7, common_kwargs["slot_dim"]], device=device)
        img = misc.print_module_summary(G, [z, c, slots])
        misc.print_module_summary(D, [img, c, slots])
    ...
    loss = dnnlib.util.construct_class_by_name(
        device=device, G=G, G_ema=G_ema, D=D, oc_type=oc_type, oc_path=oc_path, sa_size=sa_size, slot_dim=slot_dim, **loss_kwargs)  # subclass of training.loss.Loss
    phases = []
    ...
    if rank == 0:
        ...
        reals = torch.stack(
            [torch.from_numpy(training_set[i][0]) for i in range(batch_gpu)]).to(torch.float32).to("cuda")
        reals = (reals / 255).clamp(0, 1)
        slots = loss.get_slots(reals, drange=(0, 1))

        images = torch.cat([G_ema(z=z, c=c, noise_mode='const', slots=slots).cpu(
    ...
    if rank == 0:
        ...
        try:
            ...
            reals = torch.stack(
                [torch.from_numpy(training_set[i][0]) for i in range(batch_gpu)]).to(torch.float32).to("cuda")
            reals = (reals / 255).clamp(0, 1)
            stats_tfevents.add_images("images/reals", reals, 0)
    ...
    while True:
        ...
        # Save image snapshot.
        def get_random_slots(batch_size):
            indices = np.random.randint(0, len(training_set), size=batch_size)
            reals = torch.stack(
                [torch.from_numpy(training_set[i][0]) for i in indices]).to(torch.float32).to(device=device)
            reals = (reals / 255).clamp(0, 1)
            slots = loss.get_slots(reals, drange=(0, 1))
            return slots
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, noise_mode='const', slot_fn=get_random_slots).cpu()
                                for z, c in zip(grid_z, grid_c)]).numpy()
            save_image_grid(images, os.path.join(
                run_dir, f'fakes{cur_nimg//1000:06d}_random.png'), drange=[-1, 1], grid_size=grid_size)
        ...
        if cur_tick and (snapshot_data is not None):  # and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'], run_dir=run_dir, cur_nimg=cur_nimg,
                                                      dataset_kwargs=training_set_kwargs, num_gpus=num_gpus, rank=rank, device=device, G_kwargs={"slot_fn": get_random_slots})
                if rank == 0:
                    metric_main.report_metric(
                        result_dict, run_dir=run_dir, snapshot_pkl=snapshot_pkl)
                stats_metrics.update(result_dict.results)

            # Slot reproduction metric
            if rank == 0:
                slot_reproduction_error = torch.nn.MSELoss()
                errors = []
                sre_eval_loader = torch.utils.data.DataLoader(
                    training_set, batch_size=batch_gpu, shuffle=True)
                i = 0
                for reals, *_ in sre_eval_loader:
                    if i > 10_000:
                        break
                    i += batch_gpu
                    reals = reals.to(device)
                    reals = reals.to(torch.float32) / 255

                    with ManualSeed():
                        slots = loss.get_slots(reals, drange=(0, 1))
                    z = torch.randn(reals.shape[0], G_ema.z_dim).to(device)
                    c = torch.zeros(reals.shape[0], G_ema.c_dim).to(device)
                    fakes = G_ema(z, c, slots=slots)
                    with ManualSeed():
                        fake_slots = loss.get_slots(fakes, drange=(-1, 1))
                    error = slot_reproduction_error(slots, fake_slots)
                    errors.append(error)
                final_error = torch.stack(errors).mean()

                jsonl_line = json.dumps({
                    "nimg": __CUR_NIMG__.item(),
                    "error": final_error.item(),
                    "timestamp": time.time()
                })
                print(jsonl_line)
                if run_dir is not None and os.path.isdir(run_dir):
                    with open(os.path.join(run_dir, f'slot-reproduction-metric.jsonl'), 'at') as f:
                        f.write(jsonl_line + '\n')
        ...
        if stats_tfevents is not None:
            ...
            if rank == 0:
                ...
                z = torch.randn([batch_gpu, G.z_dim],
                                device=device)
                c = torch.zeros(batch_gpu)

                with torch.no_grad():
                    fakes = G(z=z, c=c, noise_mode='const', slots=slots)
                    fakes = ((fakes + 1) / 2).clamp(0, 1)

                stats_tfevents.add_images(
                    "images/fakes", fakes, global_step)
