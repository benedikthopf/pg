...
class Dataset(torch.utils.data.Dataset):
    ...
    def set_classes(self, cls_list):
        ...
        assert all(sorted(cls_list) == np.unique(
            self._raw_labels[self._raw_idx]))
    ...
    def __getitem__(self, idx):
        ...
        # if not list(image.shape) == self.image_shape:
    ...
    def resolution(self):
        assert len(self.image_shape) == 3  # CHW
        assert self.image_shape[1] == self.image_shape[2] or self.resize
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        self._image_fnames = sorted(fname for fname in self._all_fnames if
                                    (self._file_ext(fname) in PIL.Image.EXTENSION or self._file_ext(
                                        fname) == ".webp")
                                    )
